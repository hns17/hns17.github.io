...
redirect_from:
  - "/project/portfolio/SampleProject-GLGameTooL/"
tags: ["OpenGL", "Project", "Graphics"]

---

> 핵심 요약
> - 이 글은 `SampleProject_GLGameTool` 프로젝트에서 구현한 내용을 정리한다.
...
- OpenGL 3.2, GLSL 150
  ...
  - OpenGL은 2.x 버전을 클래식으로, 4.x 버전을 모던으로 정의합니다.

  - 저는 3.2를 사용하였는데, 당시 사용하던 노트북이 3.2까지만 지원했습니다. OpenGL 3.2 버전은 렌더링 방식이 Fixed 파이프라인에서 프로그래머블 파이프라인으로 완전 전환된 초기 버전이라 사용상 불편한 것들이 몇 가지 있었습니다.
...
- include 사용불가
  ...
  - 이후 버전은 모르겠지만, glsl 150은 include를 지원하지 않아 파서를 만들어야 했습니다.

- 잘못된 셰이더 지식
- 당시 인터넷 튜토리얼 문서를 통해 셰이더를 공부하였는데, 대부분이 구버전에서 이루어진 튜토리얼이라 실제 셰이더 작성 시 지원되지 않는 키워드 및 구조체가 많았습니다.
