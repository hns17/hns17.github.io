...
redirect_from:
  - "/unity/supportfeatures/00-UI-Toolkit/"
tags: ["Unity", "UI Element", "UI Toolkit", "VisualElement"]
---

> 핵심 요약
> - 이 글은 `UI Toolkit` 기능이나 라이브러리의 사용 포인트를 정리한다.
...
UI Toolkit을 사용할 경우 UI 아티스트 또한 툴킷의 빌더 기능을 사용해야 하니 제대로 된 학습이 필요하다.
...
- UI Builder를 통한 시각적 배치 및 설계가 가능하지만, 그건 어디까지나 IMGUI와 비교했을 때의 장점일 뿐 인스펙터와 에디터 뷰를 통해 작업하던 UGUI 관점에서는 전혀 장점이 되지 못한다. 오히려 UI Builder보다 UGUI가 훨씬 편하다.
...
- 관련 문서도 많지 않아서 아티스트에게 강요할 내용이 아니다
...
- 파일의 분할
  ...
  - 또한 컨트롤 스타일과 배치 등이 UXML, USS 파일로 분리되어 C# 코드 수정에 대한 컴파일을 신경 쓸 필요가 없어졌다
...
- UI Debugger 제공
  ...
  - 제작된 UI의 Visual Tree를 볼 수 있으며, 설정된 속성과 스타일 시트 등의 현재 상태를 확인할 수 있다.
  - 디버거 상 속성 값을 수정하면 수정된 값이 반영된 UI를 바로 확인할 수 있다.
  - 커스텀한 UI뿐만 아니라 기존 Unity에서 작업된 UI들의 Visual Tree도 확인할 수 있다.
    - 이는 에디터 툴 개발에 상당히 참고가 됨

- 기타 편의성 툴

  - Event Debugger를 통해 UI에서 일어나는 Event를 확인할 수 있다
...
- 뚜렷한 단점을 꼽기는 어렵다.
- 굳이 찾자면, 만들어진 지 오래된 IMGUI와 비교하면 관련 문서가 적다.
- 또한 AssetStore의 Asset들 대부분이 IMGUI로 구성되어 있다
  - VisualElement를 사용한다고 IMGUI를 사용하지 못하는 건 아니라서 크게 문제가 되지 않음
