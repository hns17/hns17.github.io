...
categories: [Tools/UnityEditor]
redirect_from:
  - "/unity/supportfeatures/00-UI-Toolkit-Event/"
tags: ["Unity", "UI Element", "UI Toolkit", "VisualElement", "UI Builder", "Event"]

---

> 핵심 요약
> - 이 글은 `UI Toolkit - Event` 기능이나 라이브러리의 사용 포인트를 정리한다.
...
- 필요에 따라 부모 요소가 자식 요소보다 먼저 반응하도록 하려면 콜백을 등록할 때 TrickleDown 옵션을 지정하여 버블업 단계가 아닌 TrickleDown 단계에서 콜백을 호출하도록 할 수 있다
...
NoTrickleDown : 버블업 단계에서 실행
TrickleDown : TrickleDown 단계에서 실행
```
